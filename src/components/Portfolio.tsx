...
export default function Portfolio({ title }: { title: string }) {
  ...
  const handleRedirect = () => {
    if (title === "Website Projects") {
      ...
      router.push("/portfolio/#wordpress");
    }
  };
          ...
          {projects.images.map((image, index) => (
            ...
            >
              <img
                src={`/portfolio/${image}`}
                alt={`${title} ${index + 1}`}
                className={`w-full aspect-[1800/820] transition-transform duration-300 ease-in-out object-cover object-center ${title === "Mobile App Projects"
