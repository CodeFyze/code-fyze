...
export default function Dropdown({ title, variant }: { title?: string, variant?: string }) {
  return (
    <span className="hover:text-blue-800 z-20">
      <DropdownMenu>
            ...
            >
              ...
              <ul className="list-disc list-inside">
                {service.technologies.map((tech, techIndex) => (
                  <Link key={techIndex} href={tech.link}><li className="text-[#7D8D9A] text-base cursor-pointer hover:scale-105 transition-transform ease-in-out duration-300 hover:font-bold hover:text-[#0E3172]">
                    {tech.text}
                  </li></Link>
                ))}
              </ul>
